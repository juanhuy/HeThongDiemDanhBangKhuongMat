...
export default function LeaveRequests({ API_BASE, showToast }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        setLoading(true);
        const res = await authFetch(`${API_BASE}/api/leave-requests`);
        if (res.ok) {
  ...
  const handleAction = async (id, action) => {
    try {
      const res = await authFetch(`${API_BASE}/api/leave-requests/${id}`, {
        method: 'PUT',
      ...
      if (res.ok) {
        ...
        setRequests((prev) => prev.map((r) => (r.id === id ? { ...r, status: action, trang_thai: action } : r)));
      } else {
              ...
              <tr key={r.id} style={{ borderBottom: '1px solid #e2edf5' }}>
                ...
                <td style={{ padding: '10px 12px' }}>
                  <span style={{
                    ...
                    background: (r.trang_thai || r.status) === 'Approved' ? '#dcfce7' : (r.trang_thai || r.status) === 'Rejected' ? '#fee2e2' : '#fef9c3',
                    color: (r.trang_thai || r.status) === 'Approved' ? '#16a34a' : (r.trang_thai || r.status) === 'Rejected' ? '#dc2626' : '#a16207',
                  }}>{r.trang_thai || r.status || 'Pending'}</span>
                </td>
                <td style={{ padding: '10px 12px' }}>
                  {((r.trang_thai || r.status) === 'Pending' || !(r.trang_thai || r.status)) && (
